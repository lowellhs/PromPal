...
Type gpu # (0 for NVIDIA GeForce RTX 2080 Ti) and hit enter to use gpu.
...
if(option == 'cpu'):
    ...
    device_name = 'NVIDIA GeForce RTX 2080 Ti GPU'
    RESULT_PATH = os.path.join(THIS_FOLDER, 'results/RTX2080Ti_results/')
    if not os.path.exists(RESULT_PATH):
        os.makedirs(RESULT_PATH)
        file_count = 1
        is_null = True
    ...
    if is_null == False:
        file_count = len(RESULT_PATH)
    result_file = os.path.join(RESULT_PATH, 'results_RTX2080Ti_#{0}.txt'.format(file_count))
    file = open(result_file, 'w') 
    write_file(file, device_name, file_count)
...
int main(int argc, char **argv)
{
  ...
  for (int counter=3; counter < 3+sizeCounter; counter++)
  {
    for (int testCounter=0; testCounter < tests; testCounter++)
    {
      ...
      float *A, *b, *x, *x_iter, *x_iter_new, *D_inv, *LU;
      float *x_lu_result, *x_bminx_result;
      float al, bet;
      ...
      cudaMallocManaged(&x_lu_result, n*sizeof(float));
      cudaMallocManaged(&x_bminx_result, n*sizeof(float));
      ...
      do {
        ...
        cublasSgemv(handle, CUBLAS_OP_N, n, n, &al, LU, n, x_iter, 1, &bet, x_lu_result, 1);
        al = 1.0f; cublasSaxpy(handle, n, &al, b, 1, x_lu_result, 1);
        al = 1.0f; cublasSgemv(handle, CUBLAS_OP_N, n, n, &al, D_inv, n, x_lu_result, 1, &bet, x_iter_new, 1);
      ...
      cudaFree(x_lu_result); cudaFree(x_bminx_result);
    }
  }
  return 0;
}
